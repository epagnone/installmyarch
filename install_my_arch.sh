...
COM=$(echo $0 | sed 's/install_my_arch_V2.sh/com-packages.cfg/')
AUR=$(echo $0 | sed 's/install_my_arch_V2.sh/aur-packages.cfg/')
...
ENV="plasma-desktop sddm sddm-kcm plasma-nm kscreen konsole dolphin dolphin-plugins ark zsh zsh-completions"
...
if [[ $(autodetect vols) ]]; then
  ...
  autodetect vgn
  # RSIZE=$(autodetect size_show root)
  # HSIZE=$(autodetect size_show home)
else
  autodetect cleanpv
  PVSIZE=$(autodetect size_show $PV)
  VGN="ARCH"
...
while [[ $g = 0 ]]; do
  ...
  " [*] Efi" "Partition:\Z4$EDISP\Zn | Format:\Z4$EFIFLAG\Zn | Mount:\Z4$EPOINT\Zn" \
  " [*] Lvm" "Partition:\Z4$PV\Zn | Group:\Z4$VGN\Zn | Size:\Z4$PVSIZE\Zn" \
  " [*] Root" "Root Volume:\Z4$LVROOT\Zn | Size:\Z4$RSIZE\Zn" \
  ...
  case $menuitem in
    ...
    IMPFLAG=$(dialog --colors --clear --title "\Z7[ IMPORT FILES ]\Zn" \
    --yesno "\n Import configuration files (home, etc and share)" 7 65 \
    3>&1 1>&2 2>&3 3>&- 
    )
    if [[ $? = 0 ]]; then
        IMPFLAG="Yes"
    ...
    --yesno "\n If efi partition is shared with other/s SO." 7 65 \
    3>&1 1>&2 2>&3 3>&- \
    )
    if [[ $? = 0 ]]; then
        EFIFLAG="Yes"
...
[[ $EFIFLAG = "No" ]] && text y "[*] root\e[0m: No se formateo EFI"
...
autodetect mountvols "$VGN-$LVROOT" "/mnt" "Root"
...
[[ -n $MDISP ]] && echo "UUID=$(lsblk -lo NAME,UUID |grep -w $MDISP |awk '{print $2}') $MPOINT ntfs-3g rw,users,umask=0022,uid=1000,gid=100 0 0" >> /mnt/etc/fstab
text g "\n [+] FSTAB Actualizado\n"
...
# Enable Bluetooth (only Native install)
if [ $TYPEFLAG = "Native" ]; then
  $CHR "systemctl enable bluetooth"
  text g "\n[+] Servicio Bluetooth habilitado\n"
fi

# enable vm tools (only Vmware install)
if [ $TYPEFLAG = "Vmware" ]; then
...
#------------------[ CHROOT CUSTOM ]---------------------
  ...
  rsync -Paq /install/custom/cbass/ /mnt/home/$USR1
  ...
  #Sincro usr/share
  rsync -Paq /install/custom/usr/share/ /mnt/usr/share
  ...
  #Sincro etc
  rsync -Paq /install/custom/etc/ /mnt/etc
  ...
  rsync -Paq /install/custom/cbass/.oh-my-zsh /mnt/root/
  ...
  cp -f /mnt/home/$USR1/.config/Neofetch/config.conf /mnt/root/.config/Neofetch/config.conf
